...
impl ConnectorCommon for Moneris {
    ...
    fn get_auth_header(&self, auth_type:&types::ConnectorAuthType)-> CustomResult<Vec<(String,String)>,errors::ConnectorError> {
        let auth: moneris::MonerisAuthType = auth_type
            .try_into()
...
impl api::PreVerify for Moneris {}
impl
    ...
    for Moneris
{
    ...
    fn handle_response(
        ...
        let response: moneris:: MonerisPaymentsResponse = res
            .response
            .parse_struct("moneris PaymentsResponse")
    ...
    > for Moneris
{
    ...
    fn handle_response(
        ...
        let response: moneris::MonerisPaymentsResponse = res
            ...
            .parse_struct("Moneris PaymentsResponse")
            .change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        logger::debug!(monerispayments_create_response=?response);
...
impl
    ...
    fn get_request_body(&self, req: &types::PaymentsAuthorizeRouterData) -> CustomResult<Option<String>,errors::ConnectorError> {
        let moneris_req =
            utils::Encode::<moneris::MonerisPaymentsRequest>::convert_and_encode(req).change_context(errors::ConnectorError::RequestEncodingFailed)?;
        Ok(Some(moneris_req))
    }
    ...
    fn handle_response(
        ...
        let response: moneris::MonerisPaymentsResponse = res.response.parse_struct("PaymentIntentResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        logger::debug!(monerispayments_create_response=?response);
        types::ResponseRouterData {
            response,
            data: data.clone(),
...
impl
    ...
    fn get_request_body(&self, req: &types::RefundsRouterData<api::Execute>) -> CustomResult<Option<String>,errors::ConnectorError> {
        let moneris_req = utils::Encode::<moneris::MonerisRefundRequest>::convert_and_encode(req).change_context(errors::ConnectorError::RequestEncodingFailed)?;
        Ok(Some(moneris_req))
    }
    ...
    fn handle_response(
        ...
        logger::debug!(target: "router::connector::moneris", response=?res);
        let response: moneris::RefundResponse = res.response.parse_struct("moneris RefundResponse").change_context(errors::ConnectorError::RequestEncodingFailed)?;
        types::ResponseRouterData {
            response,
            data: data.clone(),
            http_code: res.status_code,
...
impl
    ConnectorIntegration<api::RSync, types::RefundsData, types::RefundsResponseData> for Moneris {
    ...
    fn handle_response(
        ...
        logger::debug!(target: "router::connector::moneris", response=?res);
        let response: moneris::RefundResponse = res.response.parse_struct("moneris RefundResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        types::ResponseRouterData {
            response,
            data: data.clone(),
            http_code: res.status_code,
...
impl api::IncomingWebhook for Moneris {
...
impl services::ConnectorRedirectResponse for Moneris {
    fn get_flow_type(
        &self,
        _query_params: &str,
